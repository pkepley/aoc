...
def parse_data(data):
    ...
    for l in lines:
        ...
        if mat:
            date_times.append(datetime.strptime(mat.group(0), "%Y-%m-%d %H:%M"))
    ...
    for oi in obs_verbose:
        ...
        if "begins shift" in descr:
            ...
            obs_simple.append({"guard_id": guard_id, "chg_min": []})
...
def find_longest_time_asleep(obs):
    ...
    tot_time_asleep = [
        (k, v) for k, v in zip(tot_time_asleep.keys(), tot_time_asleep.values())
    ]
