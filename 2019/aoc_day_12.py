    ...
    for k in range(n_steps):
        for moon_1 in range(n_moons):
            for moon_2 in range(moon_1+1, n_moons):
                vel_updt = np.sign(moon_positions[moon_1,:] -
                                   moon_positions[moon_2,:]).astype(np.int)
...
if __name__ == '__main__':
    moon_positions = np.array([[-1,  0,  2],
                               [ 2,-10, -7],
                               [ 4, -8,  8],
                               [ 3,  5, -1]], dtype=np.int)
    test_result = np.array([[2,1,-3],
                            [1,-8,0],
                            [3,-6,1],
                            [2,0,4]], dtype=np.int)
