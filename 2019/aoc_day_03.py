...
def segment_intersection(seg_A, seg_B):
    ...
    if (seg_A[0][0] == seg_A[1][0]) and (seg_B[0][0] == seg_B[1][0]):
        ...
        # Vertical with same x-coords
            ...
            return [
                (seg_A[0][0], y)
                for y in range(seg_A[0][1], seg_A[1][1] + 1)
    ...
    if (seg_A[0][1] == seg_A[1][1]) and (seg_B[0][1] == seg_B[1][1]):
        ...
        # Horizontal with same y-coords
            ...
            return [
                (x, seg_A[0][1])
                for x in range(seg_A[0][0], seg_A[1][0] + 1)
...
def path_nontrivial_intersection_steps(edges_A, edges_B, origin):
    ...
    for seg_A in edges_A:
        ...
        for seg_B in edges_B:
            ...
            if intersection and intersection != [origin]:
                intersection_list.extend(intersection)
                intersection_steps_list_A.extend(
                    [steps_A + edge_length([seg_A[0], i]) for i in intersection]
                )
                intersection_steps_list_B.extend(
                    [steps_B + edge_length([seg_B[0], i]) for i in intersection]
                )
    ...
    intersection_total_steps = [
        a + b for (a, b) in zip(intersection_steps_list_A, intersection_steps_list_B)
    ]
...
def solve_part_2(seg_descr_A, seg_descr_B):
    intersection_total_steps = path_nontrivial_intersection_steps(
        path_edges(seg_descr_A), path_edges(seg_descr_B), (0, 0)
    )
...
if __name__ == "__main__":
    # Test Vertically...
    assert segment_intersection([(0, 0), (0, 10)], [(0, -3), (0, 8)]) == [
        (0, y) for y in range(0, 8 + 1)
    ]
    assert segment_intersection([(0, -3), (0, 8)], [(0, 0), (0, 10)]) == [
        (0, y) for y in range(0, 8 + 1)
    ]
    ...
    # Test Horizontally...
    assert segment_intersection([(0, 0), (10, 0)], [(-4, 0), (7, 0)]) == [
        (x, 0) for x in range(0, 7 + 1)
    ]
    assert segment_intersection([(7, 0), (-4, 0)], [(0, 0), (10, 0)]) == [
        (x, 0) for x in range(0, 7 + 1)
    ]
